...
import { FindEventOutputDto } from '../../application/dto/event-usecase.dto';
...
export class EventMapper {
  /**
   * Converts an Event entity into a plain object (DTO) to the repository
   * @param input Event entity to be converted
   * @returns Plain object representing the entity
   */
  static toObjRepository(input: Event): EventMapperProps {
   ...
   * @returns Plain object representing the entity
   */
  static toObj(input: Event): FindEventOutputDto {
    if (!input || !(input instanceof Event)) {
      throw new MapperError('Invalid Event entity provided to mapper');
    }
   ...
   */
  static toObjList(entities: Event[]): FindEventOutputDto[] {
